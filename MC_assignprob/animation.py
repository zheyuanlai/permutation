...
if __name__ == '__main__':
    d, beta = 25, 2.0
    sub_steps, full_steps = 20000, 5000
    ...
    # --- Animation setup ---
    fig, (ax_conf, ax_mag) = plt.subplots(2, 1, figsize=(10, 6), gridspec_kw={'height_ratios': [1, 1]})

    # Configuration plot
    im = ax_conf.imshow(
        np.zeros((2, d)),
        cmap=plt.cm.binary.resampled(2),
        vmin=-1, vmax=1,
        aspect='auto'
    )
    ax_conf.set_yticks([0, 1])
    ax_conf.set_yticklabels(['Standard MH', 'Projected MH'])
    ax_conf.set_xticks([])
    ax_conf.set_title('Spin configurations (1D Ising)')

    # Magnetization plot
    line_orig, = ax_mag.plot([], [], color='blue', label='Standard MH')
    line_proj, = ax_mag.plot([], [], color='red', label='Projected MH')
    ax_mag.set_xlim(0, full_steps)
    ax_mag.set_ylim(-1.1, 1.1)
    ax_mag.set_xlabel('Step')
    ax_mag.set_ylabel('Magnetization')
    ax_mag.legend(loc='lower right')
    xdata = np.arange(full_steps)

    save_interval = 2
    frames_to_animate = full_steps // save_interval

    # --- Animation function ---
    def animate(i):
        t = i * save_interval
        conf_frame = np.stack([orig_chain[t], proj_chain[t]])
        im.set_data(conf_frame)

        line_orig.set_data(xdata[:t+1], mag_orig[:t+1])
        line_proj.set_data(xdata[:t+1], mag_proj[:t+1])

        return im, line_orig, line_proj

    ani = animation.FuncAnimation(
        fig, animate, frames=frames_to_animate,
        interval=2, blit=True
    )

    ani.save('ising_config_with_magnetization.gif', writer='ffmpeg', fps=30)